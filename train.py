...
from stgcn.utils import normalize_matrix, process_adj_mat
...
def train(config_path):
    ...
    A_hat = torch.from_numpy(normalize_matrix(process_adj_mat(adj_mat, data_config['preprocess_eps'], data_config['preprocess_sigma_sq']))).to(device, dtype=torch.float)
    ...
    model = STGCN(
        ...
        model_config['spatial_in_channels'],
        model_config['spatial_out_channels'],
        model_config['temporal_out_channels'],
        model_config['temporal_kernel_dim']
        ).to(device)
...
if __name__ == '__main__':
    ...
    wandb.init(project="tugas-akhir", entity="mikhaelbelmiro", name=f'STGCN_{version}')
    train(config_path)